...
articles = fetch_news()
dates = sorted(list(set(article['publishedAt'].split('T')[0] for article in articles)), reverse=True)
selected_date = st.selectbox("Pilih Tanggal Berita yang Ingin Diparafrase:", dates)

selected_articles = [article for article in articles if article['publishedAt'].split('T')[0] == selected_date]
article_titles = ["Pilih Berita"] + [article['title'] for article in selected_articles]
selected_title = st.selectbox("Pilih Judul Berita yang Ingin Diparafrase:", article_titles)

selected_article = None
if selected_title != "Pilih Berita":
    selected_article = next((article for article in selected_articles if article['title'] == selected_title), None)
